...
import './interfaces/ISummitswapRouter01.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWETH.sol';

contract SummitswapRouter01 is ISummitswapRouter01 {
    address public immutable override factory;
    address public immutable override WETH;

    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'SummitswapRouter01: EXPIRED');
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'SummitswapRouter01: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = SummitswapLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'SummitswapRouter01: INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidity(
        ...
        require(amountA >= amountAMin, 'SummitswapRouter01: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'SummitswapRouter01: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, SummitswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'SummitswapRouter01: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, SummitswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        _swap(amounts, path, to);
    }
    ...
    {
        require(path[0] == WETH, 'SummitswapRouter01: INVALID_PATH');
        amounts = SummitswapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'SummitswapRouter01: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'SummitswapRouter01: INVALID_PATH');
        amounts = SummitswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'SummitswapRouter01: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, SummitswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'SummitswapRouter01: INVALID_PATH');
        amounts = SummitswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'SummitswapRouter01: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(SummitswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public pure override returns (uint amountIn) {
        return SummitswapLibrary.getAmountOut(amountOut, reserveIn, reserveOut);
    }
