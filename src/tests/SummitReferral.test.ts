...
import SummitReferral from "../../artifacts/src/contracts/SummitReferral.sol/SummitReferral.json";
import WETH from "../../artifacts/src/contracts/utils/WBNB.sol/WBNB.json";
import Token from "../../artifacts/src/contracts/utils/DummyToken.sol/DummyToken.json";
import SummitswapFactory from "../../artifacts/src/contracts/SummitswapFactory.sol/SummitswapFactory.json";
import SummitswapRouter02 from "../../artifacts/src/contracts/SummitswapRouter02.sol/SummitswapRouter02.json";
...
describe("Summit Referral", () => {
  ...
  it("wallet should able to recordReferral", async () => {
    ...
    const otherWalletReferrer = await summitReferral.getReferrer(otherWallet.address);
  ...
  it("LeadInfluencer should get reward if SubInfluencer getReward", async () => {
    ...
    const otherWallet2Referrer = await summitReferral.getReferrer(otherWallet2.address);
  ...
  it("tokenR rewardBalance for SubInfluencer should be 271064331144887 wei", async () => {
    const rewardBalance = await summitReferral.rewardBalance(subInfluencer.address, tokenR.address);
    assert.equal(rewardBalance.toString(), "271064331144887");
  });

  it("tokenR rewardBalance for OtherWallet2 should be 54212866228977482 wei", async () => {
    const rewardBalance = await summitReferral.rewardBalance(otherWallet2.address, tokenR.address);
