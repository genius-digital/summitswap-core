...
import SummitswapKickstarterArtifact from "@built-contracts/SummitswapKickstarter.sol/SummitswapKickstarter.json";
import { SummitswapKickstarter } from "build/typechain";
...
describe("summitswapKickstarter", () => {
  ...
  let summitswapKickstarter: SummitswapKickstarter;

  const getTimestampFromMinutes = (minutes: number) => minutes * 60;

  beforeEach(async () => {
    summitswapKickstarter = (await deployContract(owner, SummitswapKickstarterArtifact, [
      MIN_CONTRIBUTION,
      START_TIMESTAMP,
      END_TIMESTAMP,
    ])) as SummitswapKickstarter;
  });

  describe("owner", async () => {
    it("should not be nonOwner", async () => {
      assert.notEqual(await summitswapKickstarter.owner(), otherWallet.address);
    });

    it("should be owner", async () => {
      assert.equal(await summitswapKickstarter.owner(), owner.address);
    });
  });
  ...
  describe("transferOwnership", async () => {
    it("should revert when called by nonOwner", async () => {
      await expect(
        summitswapKickstarter.connect(otherWallet).transferOwnership(otherWallet.address)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });

    it("should transfer ownership to otherWallet", async () => {
      assert.equal(await summitswapKickstarter.owner(), owner.address);
      await summitswapKickstarter.transferOwnership(otherWallet.address);
      assert.equal(await summitswapKickstarter.owner(), otherWallet.address);
    });
  });
