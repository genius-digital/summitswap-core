...
describe("Summit Referral Test", () => {
  ...
  beforeEach(async () => {
    ...
    summitswapRouter02 = await deployContract(
      owner, SummitswapRouter02, [summitswapFactory.address, weth.address], { gasLimit: 4600000 }
    );
  ...
  describe("transferOwnership() to otherWallet", async () => {
    it("transferOwnership should not be called by otherWallet", async () => {
      await expect(
        summitReferral.connect(otherWallet).transferOwnership(otherWallet.address)
  ...
  describe("Account should be able to setFirstBuyFee", async () => {
    it("setFirstBuyFee with otherWallet (Not The Owner) should be reverted", async () => {
      await expect(
        summitReferral.connect(otherWallet).setFirstBuyFee(tokenA.address, 50000000)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
  ...
  describe("LeadInfluencer should be able to add SubInfluencer", async () => {
    ...
    it("addSubInfluencer _leadFee + _infFee should be 10^9", async () => {
      await expect(
        summitReferral.connect(leadInfluencer).addSubInfluencer(subInfluencer.address, (0.05 * feeDenominator) / 100, (50 * feeDenominator) / 100)
      ).to.be.revertedWith("Wrong Fee");
    });
  ...
  describe("Account Should able to swap", async () => {
    ...
    it("User will be able to swap eth to exact token", async () => {
      ...
      let otherWalletRewardBalance = await summitReferral.rewardBalance(otherWallet.address, tokenR.address);
      let leadInfRewardBalance = await summitReferral.rewardBalance(leadInfluencer.address, tokenR.address);
      let developerRewardBalance = await summitReferral.rewardBalance(owner.address, tokenR.address);
    ...
    it("User will not get reward in the second swap", async () => {
      ...
      let amount2 = await summitswapRouter02.getAmountsOut(utils.parseEther('0.1'), [weth.address, tokenA.address]);
      let amountOut2 = amount2[0];
      let amountIn2 = amount2[1];
      await summitswapRouter02.connect(otherWallet).swapETHForExactTokens(
      ...
      let totalSharedReward2 = await summitReferral.totalSharedReward(tokenR.address);
      let otherWalletRewardBalance2 = await summitReferral.rewardBalance(otherWallet.address, tokenR.address);
      let leadInfRewardBalance2 = await summitReferral.rewardBalance(leadInfluencer.address, tokenR.address);
      let developerRewardBalance2 = await summitReferral.rewardBalance(owner.address, tokenR.address);
  ...
  describe("LeadInfluencer should get reward if SubInfluencer get reward", async () => {
    ...
    it("leadInfluencer.address should get reward if subInfluencer.address get reward", async () => {
      ...
      let subInfluencerSwapList = await summitReferral.getSwapList(subInfluencer.address);
      let subInfluencerSwapInfo = subInfluencerSwapList[0];
      ...
      let leadInfluencerSwapList = await summitReferral.getSwapList(leadInfluencer.address);
      let leadInfluencerSwapInfo = leadInfluencerSwapList[0];
      ...
      let totalSharedReward = await summitReferral.totalSharedReward(tokenR.address);
      let otherWalletRewardBalance = await summitReferral.rewardBalance(otherWallet2.address, tokenR.address);
      let leadInfRewardBalance = await summitReferral.rewardBalance(leadInfluencer.address, tokenR.address);
      let subInfRewardBalance = await summitReferral.rewardBalance(subInfluencer.address, tokenR.address);
      let developerRewardBalance = await summitReferral.rewardBalance(owner.address, tokenR.address);
